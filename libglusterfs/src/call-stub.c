		       ...
		       uint8_t *dir_checksum)
{
  ...
  if (op_ret >= 0)
    {
      stub->args.checksum_cbk.file_checksum = memdup (file_checksum, 4096);
      stub->args.checksum_cbk.dir_checksum = memdup (dir_checksum, 4096);
    }
  return stub;
}
...
call_resume_unwind (call_stub_t *stub)
{
  ...
  case GF_FOP_CHECKSUM:
	...
	{
	  freee (stub->args.checksum_cbk.file_checksum);
	  freee (stub->args.checksum_cbk.dir_checksum);
	}
